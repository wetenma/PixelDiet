...
// ⭐️ 1. ViewModel -> AndroidViewModel로 변경
...
// ⭐️ 2. (application: Application) : AndroidViewModel(application)으로 변경
class SharedViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // ⭐️ 3. init 블록 추가: 앱 시작 시 실제 데이터 로드
    init {
        refreshData()
    }

    // ⭐️ 4. 실제 데이터를 로드하는 함수 (ViewModel이 Context를 Repository에 전달)
    fun refreshData() {
        viewModelScope.launch(Dispatchers.IO) { // IO 스레드에서 작업
